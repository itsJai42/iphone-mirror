# iPhone Mirror for Noctalia & Umbriel

An on-demand USB or Wi-Fi mirror application with a native Noctalia V5 bar widget plugin and Umbriel compositor integration.

Current project maintained and developed by [itsJai42](https://github.com/itsJai42). Originally built by [Daniel Lemky](https://github.com/daniellemky).
...
```sh
curl -fsSL https://github.com/itsjai42/iphone-mirror/releases/download/v0.1.0/install-online.sh | bash -s -- --release v0.1.0
```
...
- Linux with a working systemd user session, Umbriel compositor, and Noctalia V5.
...
- Tested phones:
  - iPhone 13, iOS 27.0 build 24A437, developer image 27A5228h.
  - iPhone 17 Pro Max, iOS 27 on an AMD64 CachyOS system (AMD Ryzen 9 9950X3D).

### Umbriel window rule (recommended)

To float and size the viewer window automatically when opened under Umbriel, add the following rule to `~/.config/umbriel/umbriel.toml`:

```toml
[[window_rule]]
match.title = "^iPhone — Mirror$"
default_floating = true
default_floating_size_px = { width = 400, height = 870 }
```
...
| --- | --- | --- |
| ARM64 Linux | iPhone 13, iOS 27.0 build 24A437, developer image 27A5228h | USB and Wi-Fi mirroring and input worked in manual tests. |
| AMD64 CachyOS (AMD Ryzen 9 9950X3D) | iPhone 17 Pro Max, iOS 27 | USB and Wi-Fi mirroring and input tested and verified working. |
| Intel/x86-64 Linux | Known-working iPhone 13, iOS 27.0, developer image 27A5228h | The user confirmed USB was working. Wi-Fi opened but stopped with `player-backlog`; the failure repeated. Individual input checks have not been recorded separately. Intel support remains unverified. |
| Intel/x86-64 Linux | Second test phone, iOS 17.1.1 | The current USB tunnel requires iOS 17.4 or later. Wi-Fi connected and the display service was present, but it reported zero supported media features; video startup failed. |
| Intel/x86-64 Linux | Same second phone after updating to iOS 18.7.10, developer image 27A5228h | The image mounted and the USB tunnel connected. The display service still reported zero supported media features; usable mirroring was not established. |
...
Repository: https://github.com/itsJai42/iphone-mirror (originally built by Daniel Lemky at https://github.com/daniellemky/omarchy-iphone-mirror)
...
If a dependency is missing, the installer prints a `sudo pacman -S` command. Run that command yourself, then repeat `./install.sh`. Installation needs access to Python packages unless you have configured an offline pip source.
...
Closing the viewer also stops the session. Existing window focus uses Umbriel IPC. Logs are available with:
...
Removal stops the mirror and removes only its application, command, service, and launcher files. It keeps UI configuration, phone pairing records, developer images, and unrelated integrations. The optional Noctalia plugin must be removed separately if you installed it.

## Optional Noctalia V5 plugin

Install the application first. The native Luau plugin source is in `noctalia-plugin/`. See its [installation instructions](noctalia-plugin/README.md).

The plugin provides running/error state, reactive status tracking on the Noctalia bar, and start/focus/stop actions via its panel. Installing the application does not change the bar layout or automatically enable the plugin.
...
- **itsJai42**: Current project maintainer and developer.
- **Daniel Lemky**: Original creator and author of the project ([omarchy-iphone-mirror](https://github.com/daniellemky/omarchy-iphone-mirror)).
