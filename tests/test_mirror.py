...
class SessionTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_focus_success_with_umbriel(self):
        import json
        with tempfile.TemporaryDirectory() as root:
            runtime = Runtime(Path(root)/'runtime').acquire()
            try:
                app = Mirror(runtime)
                app.player = Mock(player=Mock(pid=456, poll=Mock(return_value=None)))
                reader = AsyncMock(readline=AsyncMock(return_value=b'{"command":"focus"}\n'))
                writer = Mock(write=Mock(), drain=AsyncMock())
                async def mock_exec(*cmd, **kwargs):
                    if cmd[0] == 'umbriel' and cmd[1] == 'windows':
                        p = AsyncMock()
                        p.communicate = AsyncMock(return_value=(b'[{"id":"win-target-1","pid":456}]', b''))
                        p.returncode = 0
                        return p
                    elif cmd[0] == 'umbriel' and cmd[1] == 'msg':
                        self.assertEqual(cmd[2], 'window-focus:win-target-1')
                        p = AsyncMock()
                        p.wait = AsyncMock(return_value=0)
                        return p
                    raise ValueError(f"unexpected cmd {cmd}")

                with patch('shutil.which', return_value='/usr/local/bin/umbriel'), \
                     patch('asyncio.create_subprocess_exec', side_effect=mock_exec):
                    await app.controls(reader, writer)
                writer.write.assert_called_once()
                reply = json.loads(writer.write.call_args[0][0].decode())
                self.assertEqual(reply, {'ok': True})
            finally:
                runtime.close()

    async def test_focus_fails_when_umbriel_missing(self):
        import json
        with tempfile.TemporaryDirectory() as root:
            runtime = Runtime(Path(root)/'runtime').acquire()
            try:
